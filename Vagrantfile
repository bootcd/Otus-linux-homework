...
:inetRouter => {
  ...
  :ipaserver => {
        :box_name => "centos/7",
        :net => [
                   {ip: '192.168.255.5', adapter: 2, netmask: "255.255.255.240", virtualbox__intnet: "router-net"},
                ]
  },
  ...
  :ansibleserver => {
        :box_name => "centos/7",
        :net => [
                   {ip: '192.168.255.6', adapter: 2, netmask: "255.255.255.240", virtualbox__intnet: "router-net"},
                ]
  },
    ...
    config.vm.define boxname do |box|
        ...
        when "inetRouter"
          box.vm.provision "shell", run: "always", inline: <<-SHELL
			...
			sudo iptables -t nat -A POSTROUTING -s 192.168.255.0/28 -j MASQUERADE
			...
			ip route add 192.168.230.0/30 via 192.168.255.2
			ip route add 192.168.240.0/30 via 192.168.255.2
        ...
        when "centralRouter"
          box.vm.provision "shell", run: "always", inline: <<-SHELL
			...
			sysctl net.ipv4.conf.all.forwarding=1
            ip route add 192.168.2.0/25 via 192.168.240.2
            ip route add 192.168.1.0/25 via 192.168.230.2
			sed -i 's/PasswordAuthentication no/PasswordAuthentication yes/g' /etc/ssh/sshd_config
			service sshd restart
            SHELL
        when "centralServer"
